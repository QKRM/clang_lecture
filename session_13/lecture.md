...
5. (도전) `open`/`read`로 파일을 읽어 검색 함수와 연결할 수 있다.
...
## 3.7부: 함정 — 반환값 NULL을 확인하라

검색 함수 사고의 90%는 여기서 납니다. 세 함수 모두 **못 찾으면 NULL**을 반환하는데, 확인 없이 바로 쓰면 프로그램이 죽습니다.

```c
char *p = sbs_strchr("hello", 'z');   // 'z' 없음 → p == NULL
printf("%s\n", p);                    // 💥 NULL 역참조 → segfault (크래시)
printf("%c\n", *p);                   // 💥 마찬가지
```
...
- 12차시의 버퍼 오버플로우가 "복사 함수의 대표 사고"라면, **NULL 역참조는 검색 함수의 대표 사고**입니다.
- 도전 과제(file_ext, file_search)에서 `if (slash)`, `if (!found)` 검사가 계속 나오는 이유가 이것.
- 11차시 malloc 반환값 NULL 검사와 같은 습관 — "실패할 수 있는 함수의 반환값은 무조건 확인".

---

## 3.8부: 지금까지의 검색 함수 한눈에 보기

11차시 `memchr`부터 오늘까지, 검색 함수가 4개 쌓였습니다. "무엇을, 어디까지" 찾는지가 다릅니다.

| 함수 | 무엇을 찾나 | 어디까지 보나 | 못 찾으면 | 차시 |
|------|------------|--------------|----------|------|
| `sbs_memchr` | 바이트 1개 | `n`바이트 (\0 무시) | NULL | 11 |
| `sbs_strchr` | 문자 1개 (앞→뒤) | `\0`까지 | NULL | 13 |
| `sbs_strrchr` | 문자 1개 (뒤→앞) | `\0`까지 | NULL | 13 |
| `sbs_strnstr` | 부분 문자열 | `len`바이트와 `\0` 중 먼저 | NULL | 13 |

> 표준 C에는 `strstr`(len 제한 없는 부분 문자열 검색)도 있습니다. `strnstr`에서 `len` 조건만 빼면 `strstr`입니다 — BSD 계열이 buffer 안전을 위해 `n` 버전을 추가한 것(12차시 strlcpy가 strcpy의 안전판인 것과 같은 흐름).
...
7. **반환값 NULL 확인 없이 역참조 금지** — 검색 함수의 대표 사고
